...
rows = [r for r in range(2, ws.max_row + 1) if ws.cell(r, col["参考价值"]).value is not None]
...
for lab in order:
    items = buck.get(lab, [])
    P.append(f'<div class="sec"><h2>{lab}（共 {len(items)} 条，以下抽样 {min(SAMP,len(items))} 条）</h2>')
    for r in items[:SAMP]:
        P.append(f'''<div class="item">
